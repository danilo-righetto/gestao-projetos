...
<body>
			...
			<form action='<c:url value="/painel/instituicoes/editarInstituicao"></c:url>' method="POST" role="form">
			  ...
			  <c:choose>
			  	<c:when test="${(inst.tipoDocumento == 'cpf')}">
				  	<div class="form-group col-md-5" id="fisica1" ${(inst.tipoDocumento == "cpf" ? 
								'style="display: block;' : '')}>
					<label for="razaosocial">Nome Completo:</label> <input type="text"
							class="form-control" id="nomecompleto" value="${inst.razaosocial}" name="razaosocial" placeholder="Digite o nome" autofocus>
				    </div>
			  	</c:when>
			  	<c:otherwise>
	...
	<!-- INICIO - Listagem dos COORDENADORES vinculados -->
	<div class="section">
		<div class="container">
			<h4 style="font-family: arial; color: #4DC1FF">Usuarios - Coordenadores</h4>
			<div id="alertas"></div>
					...
					<c:choose>
						<c:when test="${not empty coordenadores}">
							<c:forEach items="${coordenadores}" var="coordenador">
								<tr>
									<td class="text-center">${coordenador.id}</td>
									<td class="text-center hidden-xs hidden-sm">${coordenador.nome}</td>
									<td class="text-center">${coordenador.usuario}</td>
									<td class="text-center hidden-xs hidden-sm">${(coordenador.realizaLogin == true ?
										'Sim' : 'Não')}</td>
									<td class="text-center hidden-xs hidden-sm">${coordenador.perfil.descricao}</td>
									<td class="text-center"><a href="editar/${coordenador.id}"><span
	...
	<!-- FIM - Listagem dos COORDENADORES vinculados -->
	<!-- INICIO - Listagem dos COLABORADORES vinculados -->
	<!-- FIM - Listagem dos COLABORADORES vinculados -->
</body>
</html>